package net.flashpunk
{
	...
	{
		/**
		 * The angle to render the graphic of the entity.
		 */
		...
		/**
		 * Renders the Entity to a BitmapData object and returns that object. If you override this for special behaviour,
		 * remember to call super.asBitmap() to create the Entity's graphic's bitmap.
		 */
		public function asBitmap():BitmapData 
		{
			setGraphicAngle();
			
			if (_graphic && _graphic is Image)
			{
				return (_graphic as Image).asBitmap();
			}
			else 
			{
				return null;
		...
		protected function setGraphicAngle():void 
		{
			if ((graphic is Image) && ((graphic as Image).angle != this.angle)) 
			{
				(graphic as Image).angle = this.angle;
			}
		}
