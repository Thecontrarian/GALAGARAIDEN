package net.flashpunk.graphics 
{
	...
	public class Image extends Graphic
	{
		...
		/** @private Returns image as BitmapData.*/
		public function asBitmap():BitmapData
		{
			// quit if no graphic is assigned
			if (!_buffer) return null;
			var target:BitmapData = new BitmapData( 
													_bufferRect.width,_bufferRect.height, true, 0);
													//_bufferRect.width * Math.abs(Math.cos((angle % 360) * FP.RAD)) + _bufferRect.height * Math.abs(Math.sin((angle % 360) * FP.RAD)),
													//_bufferRect.width * Math.abs(Math.sin((angle % 360) * FP.RAD)) + _bufferRect.height * Math.abs(Math.cos((angle % 360) * FP.RAD)),  true, 0);

			// determine drawing location
			var newPoint:Point = new Point();
			newPoint.x = x;// + (target.width - _bufferRect.width) / 2;
			//newPoint.x = x + (target.width - _bufferRect.width) / 2;
			newPoint.y = y;// + (target.height - _bufferRect.height) / 2;
			//newPoint.y = y  + (target.height - _bufferRect.height) / 2;
			...
			if (angle == 0 && scaleX * scale == 1 && scaleY * scale == 1 && !blend)
			{
				target.copyPixels(_buffer, _bufferRect, newPoint, null, null, true);
				return target;
			}
			...
			_matrix.ty = -originY * _matrix.d;
			if (angle != 0) _matrix.rotate(angle * FP.RAD);
			_matrix.tx += originX + newPoint.x;
			_matrix.ty += originY + newPoint.y;
			_bitmap.smoothing = smooth;
			target.draw(_bitmap, _matrix, null, blend, null, smooth);
			return target;
		}
